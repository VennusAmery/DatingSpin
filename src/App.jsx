...
const TABS=[{id:'roulette',label:'Ruleta',emoji:'🎰'},{id:'icebreaker',label:'Rompehielos',emoji:'💬'},{id:'ideas',label:'Ideas',emoji:'💡'},{id:'planner',label:'Planear',emoji:'🗓'},{id:'events',label:'Eventos',emoji:'📅'},{id:'diary',label:'Diario',emoji:'📖'}];
...
function SubPanel({activity,userLocation,onNeedLocation}){
  ...
  if(activity.id==='cinema')return<CinemaView userLocation={userLocation} onNeedLocation={onNeedLocation}/>;
...
export default function App(){
  ...
  const[disabledIds,setDisabledIds] = useLocalStorage('spindocky-disabled',[]);
  ...
  const doSpin = () => {
    if(spinning || allActivities.length === 0) return;
    setShowSub(false); setActIdx(null); setSpinning(true);
    spinRef.current += 1; setTrig(spinRef.current);
  };
  ...
  const removeIdea = id => {
    ...
    if(actIdx!==null&&allActivities[actIdx]?.id===`idea_${id}`){ setActIdx(null); setShowSub(false); }
  };

  const activity = actIdx !== null ? allActivities[actIdx] : null;
  const allActivitiesUnfiltered = [...BASE_ACTIVITIES, ...customIdeas.map(ideaToActivity)];
      ...
      <div style={{width:'100vw',height:'100dvh',display:'flex',flexDirection:'column',background:C.bg,overflow:'hidden',maxWidth:1100,margin:'0 auto'}}>
        ...
        <header style={{background:'white',borderBottom:`3px solid ${C.ink}`,height:52,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 14px',flexShrink:0,boxShadow:`0 3px 0 ${C.grayLt}`,zIndex:50}}>
          <div style={{display:'flex',alignItems:'center',gap:9}}>
            <img src="/logo.png" alt="SpinDate" style={{width:34,height:34,borderRadius:10,border:'2px solid #1A1A1A',boxShadow:'0 2px 0 #1A1A1A',objectFit:'cover'}}/>
            <div>
              <div style={{fontFamily:'Fredoka One',fontSize:18,color:C.ink,lineHeight:1}}>Spin<span style={{color:C.blue}}>Date 🐧</span></div>
              <div style={{fontFamily:'Nunito',fontSize:8,fontWeight:800,color:'#AAA',letterSpacing:'0.06em',textTransform:'uppercase'}}>Date Roulette </div>
            </div>
          </div>
        ...
        <div style={{flex:1,overflow:'hidden'}}>
          ...
          {tab==='roulette'&&(
            <div style={{height:'100%',display:'grid',gridTemplateColumns:'1fr 1fr',overflow:'hidden'}}>
              ...
              <div style={{borderRight:`2px solid ${C.grayLt}`,display:'flex',flexDirection:'column',alignItems:'center',padding:'10px 12px 8px',gap:8,overflowY:'auto'}}>
                ...
                <div style={{textAlign:'center',width:'100%'}}>
                  ...
                  <p style={{fontFamily:'Nunito',fontWeight:600,fontSize:10,color:'#AAA',margin:'2px 0 0'}}>
                    {allActivities.length} opciones activas · Gira 
                  </p>
                </div>
                ...
                <div style={{display:'flex',gap:7,width:'100%',maxWidth:272}}>
                  <button className="btn3d" onClick={doSpin} disabled={spinning||allActivities.length===0}
                    ...
                    {spinning?'🌀 Pensando...':'🎰 ¡Girar!'}
                  </button>
                  {actIdx!==null&&!spinning&&(
                    <button className="btn3d" onClick={doSpin} title="Re-spin"
                      style={{background:'#FFB085',color:C.ink,fontSize:17,padding:'10px 12px',width:46}}>🔄</button>
                  )}
                </div>
                ...
                {activity&&showSub&&(
                  <div style={{background:activity.cardColor,border:`2px solid ${C.ink}`,borderRadius:12,padding:'8px 12px',width:'100%',maxWidth:272,display:'flex',alignItems:'center',gap:9,boxShadow:`0 3px 0 ${C.ink}`,animation:'popIn 0.4s cubic-bezier(0.34,1.56,0.64,1) both',flexShrink:0}}>
                    ...
                    <div style={{minWidth:0}}>
                      <div style={{fontFamily:'Nunito',fontSize:9,fontWeight:800,color:'#AAA',textTransform:'uppercase',letterSpacing:'0.07em'}}>Opción elegida! 🎉</div>
                ...
                <div style={{width:'100%',maxWidth:272,borderTop:`1.5px solid ${C.grayLt}`,paddingTop:8}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                    <div style={{fontFamily:'Nunito',fontWeight:900,fontSize:10,color:'#AAA',letterSpacing:'0.1em',textTransform:'uppercase'}}>
                      Opciones ({allActivities.length}/{allActivitiesUnfiltered.length})
                    </div>
                    <div style={{display:'flex',gap:6,alignItems:'center'}}>
                      {disabledIds.length>0&&(
                        <button onClick={()=>setDisabledIds([])}
                          style={{background:'transparent',border:'none',cursor:'pointer',fontFamily:'Nunito',fontWeight:700,fontSize:10,color:C.blue}}>
                          Activar todas
                        </button>
                      )}
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:4,maxHeight:'calc(3 * 40px)',overflowY:'auto',paddingRight:2}}>
                    {allActivitiesUnfiltered.map(a => {
                      const isOff    = disabledIds.includes(a.id);
                      const isActive = activity?.id === a.id;
                      const isCustom = a.id.startsWith('idea_');
                      return (
                        <div key={a.id} style={{background:isActive?a.cardColor:'white',border:`1.5px solid ${isActive?a.cardBorder:C.grayLt}`,borderRadius:10,padding:'5px 9px',display:'flex',alignItems:'center',gap:6,opacity:isOff?0.4:1,transition:'all 0.15s'}}>

                          {/* ☑ Checkbox — activa/desactiva en la ruleta */}
                          <button onClick={()=>toggleDisabled(a.id)} title={isOff?'Agregar a ruleta':'Quitar de ruleta'}
                            style={{background:'transparent',border:'none',cursor:'pointer',fontSize:15,padding:0,lineHeight:1,flexShrink:0,color:isOff?'#CCC':C.blue}}>
                            {isOff?'☐':'☑'}
                          </button>

                          <span style={{fontSize:14,flexShrink:0}}>{a.emoji}</span>

                          {/* Nombre — selección directa */}
                          <button onClick={()=>{ if(!isOff) selectDirect(a.id); }}
                            style={{background:'transparent',border:'none',cursor:isOff?'default':'pointer',fontFamily:'Nunito',fontWeight:700,fontSize:11,color:isOff?'#CCC':C.ink,flex:1,textAlign:'left',padding:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}
                            title={isOff?'Activar primero':'Ver opciones directamente'}>
                            {a.name}
                          </button>

                          {/* Badge sub-ruleta */}
                          {(a.hasSubRoulette||['cinema','home','dinner','food'].includes(a.id))&&(
                            <span style={{fontSize:10,flexShrink:0,color:'#CCC'}} title="Tiene sub-ruleta">🎲</span>
                          )}

                          {/* Editar/eliminar ideas custom */}
                          {isCustom&&(
                            <button onClick={()=>{ const idea=customIdeas.find(i=>`idea_${i.id}`===a.id); if(idea)setIdeaModal({...idea}); }}
                              style={{background:'transparent',border:'none',cursor:'pointer',color:'#CCC',fontSize:12,padding:'1px 2px',lineHeight:1,flexShrink:0}}
                              onMouseEnter={e=>e.target.style.color='#555'} onMouseLeave={e=>e.target.style.color='#CCC'}>✏️</button>
                          )}
                          {isCustom&&(
                            <button onClick={()=>removeIdea(parseInt(a.id.replace('idea_','')))}
                              style={{background:'transparent',border:'none',cursor:'pointer',color:'#DDD',fontSize:14,padding:'1px 2px',lineHeight:1,flexShrink:0}}
                              onMouseEnter={e=>e.target.style.color='#FF6B6B'} onMouseLeave={e=>e.target.style.color='#DDD'}>×</button>
                          )}
                        </div>
                      );
              ...
              <div style={{display:'flex',flexDirection:'column',padding:'10px 12px 8px',overflowY:'auto',gap:8}}>
                ...
                <div style={{flex:1,minHeight:0,opacity:showSub?1:0.25,filter:showSub?'none':'grayscale(1)',transition:'opacity 0.4s, filter 0.4s',display:'flex',flexDirection:'column',pointerEvents:showSub?'auto':'none'}}>
          ...
          {tab==='events'&&<div style={{height:'100%',padding:'12px',display:'flex',flexDirection:'column',overflow:'hidden'}}><EventsPage/></div>}
          {tab==='diary'&&<div style={{height:'100%',padding:'12px',overflowY:'auto'}}><DiaryPage/></div>}
          {tab==='ideas'&&<div style={{height:'100%',padding:'12px',overflowY:'auto'}}><IdeasPage/></div>}
        </div>
      </div>
